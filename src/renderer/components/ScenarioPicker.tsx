...
import { FolderOpen, LayoutTemplate, ArrowRight, ListChecks, Search } from 'lucide-react';
...
export function ScenarioPicker() {
  ...
  useEffect(() => {
    loadCatalogEntries({ includeStubs: false }).then(setAllEntries);
  }, []);

  const filtered = filterCatalogEntries(allEntries, search);
    ...
    <div className="max-w-2xl mx-auto px-8 py-10">
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full pl-9 pr-3 py-2 text-sm border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/40"
        />
      </div>

      {folderKeys.length === 0 && search && (
        <p className="text-sm text-slate-400 text-center py-8">Brak wyników dla „{search}"</p>
      )}
